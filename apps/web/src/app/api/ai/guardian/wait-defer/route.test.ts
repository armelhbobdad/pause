...
function createRequest(body: Record<string, unknown>): Request {
  return new Request("http://localhost/api/ai/guardian/wait-defer", {
    method: "POST",
...
describe("Wait-Defer Route Handler", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('records outcome "wait" and status "feedback_received"', async () => {
    const response = await POST(createRequest(validBody));
  ...
  it("interaction with null outcome succeeds", async () => {
    mocks.interactionResult = [
      {
        id: "int-123",
        userId: "user-1",
