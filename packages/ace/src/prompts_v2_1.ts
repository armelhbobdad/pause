...
export class PromptManager {
   ...
   */
  constructor(defaultVersion = "2.1") {
  ...
  if (role === "agent") {
    ...
    if ("quality_check" in data) {
      ...
      metrics.completeness =
        (Number(qc.addresses_question) +
          Number(qc.reasoning_complete) +
          Number(qc.citations_provided)) /
        3.0;
    }
 ...
 */
export function comparePromptVersions(role = "agent"): Record<string, any> {
...
function calculateSimilarity(str1: string, str2: string): number {
  const words1 = new Set(str1.toLowerCase().split(WHITESPACE_REGEX));
  const words2 = new Set(str2.toLowerCase().split(WHITESPACE_REGEX));
