 ...
 * Please use prompts_v2_1.ts for new projects, which includes:
...
export const AGENT_V2_PROMPT = `# Identity and Metadata
...
export const GENERATOR_V2_PROMPT = AGENT_V2_PROMPT;

// ================================
// REFLECTOR PROMPT - VERSION 2.0
// ================================

export const REFLECTOR_V2_PROMPT = `# Identity and Metadata
...
export const SKILL_MANAGER_V2_PROMPT = `# Identity and Metadata
...
- **Specific Tools**: If reflection mentions specific tools, create strategy using tools specifically
...
// Backward compatibility alias
export const CURATOR_V2_PROMPT = SKILL_MANAGER_V2_PROMPT;
...
export const AGENT_MATH_PROMPT = `# Identity and Metadata
...
// Backward compatibility alias
export const GENERATOR_MATH_PROMPT = AGENT_MATH_PROMPT;

// Code-specific Agent
export const AGENT_CODE_PROMPT = `# Identity and Metadata
You are ACE Code Agent v2.0, specialized in software development.
Prompt Version: 2.0.0-code
Language Preference: TypeScript (unless specified)
...
// Backward compatibility alias
export const GENERATOR_CODE_PROMPT = AGENT_CODE_PROMPT;
...
export class PromptManager {
   ...
   *   const prompt = manager.getAgentPrompt({ domain: "math", version: "2.0" });
   */
  ...
  private readonly usageStats: Record<string, number> = {};

  constructor(defaultVersion = "2.0") {
  ...
  getAgentPrompt(options?: { domain?: string; version?: string }): string {
    ...
    let promptKey = version;
    if (domain && PromptManager.PROMPTS.agent[`${version}-${domain}`]) {
      promptKey = `${version}-${domain}`;
    }
  ...
  getReflectorPrompt(version?: string): string {
    ...
    this._trackUsage(`reflector-${ver}`);
    return prompt;
  }
  ...
  getSkillManagerPrompt(version?: string): string {
    ...
    if (!prompt) {
      throw new Error(`No skill manager prompt found for version ${ver}`);
    }

    this._trackUsage(`skill_manager-${ver}`);
    return prompt;
  }

  private _trackUsage(promptId: string): void {
  ...
  static listAvailableVersions(): PromptVersions {
    return {
      agent: { ...PromptManager.PROMPTS.agent },
      reflector: { ...PromptManager.PROMPTS.reflector },
      skill_manager: { ...PromptManager.PROMPTS.skill_manager },
    };
  }
}
...
\`\`\`typescript
// Old (v1)
import { AGENT_PROMPT, REFLECTOR_PROMPT, SKILL_MANAGER_PROMPT } from './prompts.js';

// New (v2)
import { PromptManager } from './prompts_v2.js';

const manager = new PromptManager('2.0');
...
\`\`\`typescript
// Math-specific agent
const mathPrompt = manager.getAgentPrompt({ domain: 'math' });

// Code-specific agent
const codePrompt = manager.getAgentPrompt({ domain: 'code' });
\`\`\`

## Custom Prompts with v2 Structure

\`\`\`typescript
import { AGENT_V2_PROMPT } from './prompts_v2.js';

// Use v2 as template
const customPrompt = AGENT_V2_PROMPT.replace(
